...
### Haemodynamic Pressures (2×2 Grid)
...
- **Thermodilution**: CO (L/min) and CI (L/min/m²)
...
- **PVR (Pulmonary Vascular Resistance)**: Calculated from PA, PCWP, and CO
  - Formula: (PA mean - PCWP mean) / CO × 80
  - Color coded: Green if <3 Wood units, Red if ≥3
...
- Laboratory values (Hb, lactate, mixed venous O₂)
...
RHCCardLayout.tsx renders card (React component)
    ↓
html2canvas captures at 3.125× scale (300 DPI)
    ↓
...
- ✅ `src/utils/rhcCardExport.ts` - Export utility with validation
- ✅ `src/sidepanel/components/results/RHCCardLayout.tsx` - Card layout component
- ✅ `src/sidepanel/components/results/RightHeartCathDisplay.tsx` - Integrated export button
...
✅ Chrome/Edge: Full support (extension platform)
✅ Canvas API: Native support
✅ High-resolution rendering: Supported
✅ Download API: Supported

## Privacy & Security

- ✅ **100% local-first**: No cloud uploads
- ✅ **No external API calls**: Everything runs in browser
- ✅ **Temporary DOM elements**: Cleaned up immediately after capture
- ✅ **No data persistence**: Images not cached or stored by extension
- ✅ **User-controlled downloads**: Standard browser download prompt
...
| Validation | ✅ Pre-export checks | ✅ Pre-export checks |
| One-click export | ✅ | ✅ |
| High-res output | ✅ | ✅ |
